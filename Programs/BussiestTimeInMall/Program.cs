...
diff = 6
maxDiff = Max(diff, maxDiff); //10
result = tS;
visitors = 0;
*/

namespace BussiestTimeInMall
{
    class Program
    {
            ...
            for(int i = 1; i < r; i++)
            {
                ...
                else
                {
                    if((data[i][2] == 1))
                    {
                        int temp = visitors;
                        diff = data[i][1] + temp;
                        maxDiff = Math.Max(diff - temp,maxDiff);
                        visitors += data[i][1]; 
                    }
                    else
                    {
        ...
        {
            int[][] data = new int[][]{
                    new int[3]{1487799425, 14, 1}, 
                    new int[3]{1487799425, 4,  0},
                    new int[3]{1487799425, 2,  0},
                    new int[3]{1487800378, 10, 1},
                    new int[3]{1487801478, 18, 0},
                    new int[3]{1487801478, 18, 1},
                    new int[3]{1487901013, 1,  0},
                    new int[3]{1487901211, 7,  1},
                    new int[3]{1487901211, 7,  0}
            };
